use super::super::iterator::GeoFilterOp;
use super::{InnerNodesView, LeafOffsetsView, Segment, LEAF_ENTRY_SIZE, LEAF_HEADER_SIZE};
...
impl<'a> CompactedQueryIterator<'a> {
    ...
    pub fn new_radius(
        ...
        let bbox_min_lat = super::super::point::encode_lat(bb_min_lat);
        let bbox_max_lat = super::super::point::encode_lat(bb_max_lat);
        let bbox_min_lon = super::super::point::encode_lon(bb_min_lon);
        let bbox_max_lon = super::super::point::encode_lon(bb_max_lon);
    ...
    fn next(&mut self) -> Option<u64> {
        loop {
            ...
            if self.leaf_scan.is_some() {
                if let Some(doc_id) = self.next_from_leaf_scan() {
                    return Some(doc_id);
                }
            }
...
fn cell_fully_inside_radius(
    ...
    let min_lat_f = super::super::point::decode_lat(cell_min_lat);
    let max_lat_f = super::super::point::decode_lat(cell_max_lat);
    let min_lon_f = super::super::point::decode_lon(cell_min_lon);
    let max_lon_f = super::super::point::decode_lon(cell_max_lon);
    ...
    for (lat_enc, lon_enc) in corners {
        let lat = super::super::point::decode_lat(lat_enc);
        let lon = super::super::point::decode_lon(lon_enc);
...
mod tests {
    ...
    fn test_cell_fully_inside_radius_conservative() {
        ...
        let cell_min_lat = point::encode_lat(60.0);
        let cell_max_lat = point::encode_lat(61.0);
        let cell_min_lon = point::encode_lon(-10.0);
        let cell_max_lon = point::encode_lon(10.0);
        ...
        let d_corner = haversine_distance(
            center_lat,
            center_lon,
            point::decode_lat(cell_min_lat),
            point::decode_lon(cell_max_lon),
        );
