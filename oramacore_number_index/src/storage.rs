...
use crate::indexer::IndexedValue;
use crate::info::{IndexInfo, IntegrityCheck, IntegrityCheckResult};
use crate::io::{ensure_version_dir, read_current, sync_dir, write_current_atomic};
use crate::iterator::{FilterHandle, FilterOp, SortHandle, SortOrder};
use crate::key::IndexableNumber;
use crate::live::{LiveLayer, LiveSnapshot};
use crate::merge::{sorted_merge, sorted_merge_doc_ids};
...
/// # Example
///
/// Use [`NumberIndexer`](crate::NumberIndexer) to extract values from JSON, then
...
impl<T: IndexableNumber> NumberStorage<T> {
    ...
    /// # Example
    /// ```ignore
    /// use oramacore_number_index::{NumberIndexer, NumberStorage, Threshold, SortOrder};
    /// use serde_json::json;
    /// use std::path::PathBuf;
    ///
    /// let index: NumberStorage<u64> = NumberStorage::new(
    ...
    pub fn info(&self) -> IndexInfo {
        ...
        IndexInfo {
            format_version: crate::io::FORMAT_VERSION,
...
mod tests {
    ...
    fn test_info_empty_index() {
        ...
        let info = index.info();
        assert_eq!(info.format_version, crate::io::FORMAT_VERSION);
