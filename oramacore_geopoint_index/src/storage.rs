use crate::compacted::CompactedVersion;
use crate::compacted::build::build_bkd;
use crate::config::Threshold;
use crate::indexer::IndexedValue;
use crate::info::{CheckStatus, IndexInfo, IntegrityCheck, IntegrityCheckResult};
use crate::io::{
    FORMAT_VERSION, copy_dir_contents, ensure_segment_subdir, ensure_version_dir,
    list_version_dirs, read_current, read_manifest, remove_version_dir, segment_subdir, sync_dir,
    version_dir, write_current_atomic, write_manifest, write_u64_slice,
};
use crate::iterator::{FilterData, GeoFilterOp};
use crate::live::{LiveLayer, LiveOp};
use crate::mmap_vec::{MmapVecWriter, dedup_sorted, retain_in_place};
...
impl GeoPointStorage {
    ...
    fn hot_compact(
        ...
        snapshot: &crate::live::LiveSnapshot,
        all_deletes: Vec<u64>,
    ) -> Result<()> {
    ...
    pub fn cleanup(&self) {
        ...
        for version_id in version_ids {
            if version_id != current_version_id
                && let Err(e) = remove_version_dir(&self.base_path, version_id)
            {
