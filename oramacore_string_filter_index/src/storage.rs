...
impl StringFilterStorage {
    ...
    pub fn insert(&self, indexed_value: &crate::indexer::IndexedValue, doc_id: u64) {
        match indexed_value {
            crate::indexer::IndexedValue::Plain(s) => {
                let mut live = self.live.write().unwrap();
                live.insert(s, doc_id);
            }
            crate::indexer::IndexedValue::Array(strings) => {
                let mut live = self.live.write().unwrap();
                for s in strings {
                    live.insert(s, doc_id);
                }
